...
def create_notebook_cells(sql_script):
    cells = []
    current_markdown_lines = []
    current_sql_lines = []
    
    # Split script into blocks based on lines starting with '-- ' (comment followed by space)
    # or lines that are purely SQL commands.
    # A blank line will also serve as a separator for SQL blocks.

    script_lines = sql_script.split('\\n')
    
    idx = 0
    while idx < len(script_lines):
        line = script_lines[idx]
        stripped_line = line.strip()

        if stripped_line.startswith('--'):
            # Collect all consecutive comment lines for a markdown cell
            if current_sql_lines: # Flush previous SQL cell
                cells.append({
                    "cell_type": "code",
                    "execution_count": None,
                    "metadata": {"language_id": "sql"},
                    "outputs": [],
            ...
            idx += 1
            while idx < len(script_lines) and script_lines[idx].strip().startswith('--'):
                comment_block.append(script_lines[idx].strip())
                idx += 1
            cells.append({
                "cell_type": "markdown",
                "metadata": {},
            ...
            # We've already incremented idx in the inner loop, so continue
            continue 
        elif stripped_line: # It's an SQL line
            if current_markdown_lines: # Flush previous markdown cell
                 cells.append({
                    "cell_type": "markdown",
                    "metadata": {},
            ...
            sql_block = [line] # Keep original spacing/indentation
            idx += 1
            # Collect SQL lines until a comment, a blank line, or end of script
            while idx < len(script_lines):
                next_line = script_lines[idx]
                next_stripped_line = next_line.strip()
                if not next_stripped_line or next_stripped_line.startswith('--'):
                    break # End of SQL block
                sql_block.append(next_line)
                idx += 1
            
            # Group related SQL statements (e.g., multiple CREATEs, or a USE followed by CREATEs)
            # This specific logic might need to be more sophisticated for very complex scripts,
            # but for this one, grouping by blank lines or comments should be a good start.
            cells.append({
                "cell_type": "code",
                "execution_count": None,
                "metadata": {"language_id": "sql"},
                "outputs": [],
                "source": '\\n'.join(sql_block).strip() # Join lines, then strip leading/trailing whitespace from the block
            })
            # We've already incremented idx appropriately
            continue
        else: # Empty line, just advance
            idx += 1

    # Final flush if any SQL lines are pending (e.g., script ends with SQL)
    ...
    if current_markdown_lines: # Should ideally be handled, but as a safeguard
        cells.append({
            "cell_type": "markdown",
            "metadata": {},
            "source": '\\n'.join(current_markdown_lines)
    ...
    # Ensure source is a list of strings for each cell, as per .ipynb format
    for cell in cells:
        if isinstance(cell["source"], str):
            ...
            if cell["source"] and cell["source"][-1].endswith('\\n'):
                cell["source"][-1] = cell["source"][-1][:-2]


    return [cell for cell in cells if cell["source"]] # Remove empty cells

notebook = {
    "cells": create_notebook_cells(sql_script_content),
    "metadata": {
        "kernelspec": {
...
output_notebook_dir = "snowflake_sql"
output_notebook_filename = "snowflake_sql_notebook.ipynb"
output_notebook_path = os.path.join(output_notebook_dir, output_notebook_filename)

# Create the directory if it doesn't exist
if not os.path.exists(output_notebook_dir):
    os.makedirs(output_notebook_dir)
    print(f"Directory '{output_notebook_dir}' created.")

with open(output_notebook_path, 'w') as f:
    json.dump(notebook, f, indent=1, ensure_ascii=False)

print(f"Notebook '{output_notebook_path}' created successfully.") 